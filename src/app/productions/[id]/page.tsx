 ...
 * - For other filmmakers: Shows production details and apply button
 */
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '2rem' }}>
      ...
      <div style={{ marginBottom: '2rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
          <div>
            ...
            <p style={{ color: '#666', fontSize: '0.95rem' }}>Code: {production.project_code}</p>
          </div>
          {isCreator && (
            <button
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', marginTop: '2rem' }}>
          {production.shoot_location && (
            <div>
              ...
              <p>{production.shoot_location}</p>
            </div>
          )}
          {production.shoot_start_date && (
            <div>
              <strong>Shoot Dates</strong>
              <p>{production.shoot_start_date} to {production.shoot_end_date}</p>
            </div>
          )}
          {production.is_paid && (
            <div>
              <strong>Budget</strong>
              <p>${production.budget_min}k - ${production.budget_max}k</p>
            </div>
          )}
          <div>
            <strong>Status</strong>
            <p>{production.applications_open ? '🟢 Hiring' : '🔴 Closed'}</p>
          </div>
        </div>
        ...
        {production.filmmakers && (
          <div style={{ background: '#f9f9f9', padding: '1rem', borderRadius: '4px', marginTop: '2rem' }}>
            ...
            <p>{production.filmmakers.name}</p>
          </div>
        )}
      </div>
      ...
      <div style={{ marginTop: '2rem' }}>
        <h2>Roles Available ({roles.length})</h2>
            ...
            {roles.map((role) => (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                  ...
                  {!isCreator && production.applications_open && (
                    <button
                      onClick={() => {
